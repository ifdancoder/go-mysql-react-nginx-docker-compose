...
Необходим конфигурационный файл окружения .env ([Пример .env](./nginx/.env.example), новый .env необходимо разместить в той же директории, что и пример) для удобства подстановки Nginx хоста в конфигурации. Билд и запуск происходит посредством файла [/nginx/start.sh](./nginx/start.sh).
